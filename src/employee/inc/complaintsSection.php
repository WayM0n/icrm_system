<?php
    ...
    $pend_status = 0;
    $pending_query ="SELECT COUNT(comp_status) as pending FROM `user_complaints` where comp_status=:status";
    $prep_pending_query = $pdo_com->prepare($pending_query);
    $prep_pending_query->bindParam(':status',$pend_status);
    $prep_pending_query->execute();
    $pending_data = $prep_pending_query->fetch();
?>
<section id="user_Cards" class="mx-3 my-2">
            ...
            <div class="col-lg-3 col-sm-6 my-4">
              <div id="user-card-primary" class="card shadow-lg">
                  <div class="card-body">
                    ...
                    <small class=" fw-semibold">23.6k Customers satisfied</small>
                  </div>
              </div>                  
            </div>
            <div class="col-lg-3 col-sm-6 mb-4">
              <div id="user-card-info" class="card shadow-lg">
                  <div class="card-body">
                    ...
                    <h3 class="card-title text-nowrap mb-2"><span class="fa fa-exclamation-circle"></span>
                    <?=
                        $pending_data['0'];
                    ?>
                    </h3>
                    <small class=" fw-semibold">Purchase by 2.5k Customers</small>
                  </div>
              </div>                  
            </div>
            <div class="col-lg-3 col-sm-6 mb-4">
              <div id="user-card-green" class="card shadow-lg">
                  <div class="card-body">
                    ...
                    <small class="fw-semibold">5.6% decrease in Complaints</small>
                  </div>
              </div>                  
            </div>
            <div class="col-lg-3 col-sm-6 mb-4">
              <div id="user-card-light" class="card shadow-lg">
                  <div class="card-body">
                    ...
                    <small class="fw-semibold">23% decreased last month</small>
                  </div>
              </div>                  
            </div>
        </div>
    ...
    <?php
        $i = 0;
        $status=0;
        $sql_limit = "SELECT *,icrm_customers.user_details.f_name,icrm_customers.user_details.l_name FROM `user_complaints` join icrm_customers.user_details on submitted_by = icrm_customers.user_details.id WHERE comp_status=:status ORDER BY raised_on desc";
        $sql_limit_query = $pdo_com->prepare($sql_limit);
        $sql_limit_query->bindParam(':status',$status);
                                ...
                                <table class="table table-borderless table-hover table-striped align-middle text-center">
                                    ...
                                    foreach ($result_select as $value) {
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php if($value['comp_status']){?>
                                                    <button class="btn btn-primary disabled" role="button">Solve</button>
                                                <?php
                                                }
                                                else{
                                                ?>
                                                <form action="solveComplaint.php" method="post">
                                                    <input type="hidden" name="c_id" value="<?= $value['c_id'] ?> ">
                                                    <input type="hidden" name="title" value="<?= $value['title'] ?> ">
                                                    <button class="btn btn-primary " name="btnSolution" role="button">Solve</button>
                                                    </form>
                                                <?php
                                                } ?>
                                            </td>
